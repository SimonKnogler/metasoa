...
# Single switch for Wen-style mapping
USE_WEN_MAPPING = True   # toggles Wen-style direction blending
if CHECK_MODE:
...
def run_trial(
    ...
    while clk.getTime() < total_motion_duration and resp_shape is None:
        ...
        if USE_WEN_MAPPING:
            # --- WEN-STYLE DIRECTION BLEND ---
            controlRatio = float(np.clip(prop, 0.0, 1.0))

            # Mouse instantaneous direction & speed
            vm = np.array([dx, dy], dtype=float)
            mouse_speed = float(np.linalg.norm(vm))
            if mouse_speed > 0:
                um = vm / mouse_speed
            else:
                um = np.array([0.0, 0.0], dtype=float)

            # Template unit directions (from current snippet frame)
            vt_tmp = np.array([target_ou_dx, target_ou_dy], dtype=float)
            vd_tmp = np.array([distractor_ou_dx, distractor_ou_dy], dtype=float)
            utmp_t = vt_tmp / (np.linalg.norm(vt_tmp) + 1e-9)
            utmp_d = vd_tmp / (np.linalg.norm(vd_tmp) + 1e-9)

            def blend_angle(u_self, u_tmp, w_self):
                """Rotate self-direction toward template by shortest arc; w_self ∈ [0,1]."""
                a_self = math.atan2(u_self[1], u_self[0]) if np.any(u_self) else 0.0
                a_tmp  = math.atan2(u_tmp[1],  u_tmp[0])  if np.any(u_tmp)  else 0.0
                d = a_tmp - a_self
                d = (d + math.pi) % (2*math.pi) - math.pi  # wrap to (−π, π]
                a_blend = a_self + (1.0 - w_self) * d
                return np.array([math.cos(a_blend), math.sin(a_blend)], dtype=float)

            # Use separate ratios for target (controllable) vs distractor (other)
            controlRatio_t = controlRatio
            controlRatio_d = 0.0

            # Blended directions (unit vectors)
            u_t = blend_angle(um, utmp_t, controlRatio_t)
            u_d = blend_angle(um, utmp_d, controlRatio_d)

            # Step length from mouse speed
            step_len = mouse_speed

            # Construct displayed velocities (pre-filter)
            vt_new = u_t * step_len
            vd_new = u_d * step_len

            # OPTIONAL: gentle low-pass for visual smoothness
            ALPHA = 0.25  # snappier than v2 (0.5)
            vt = ALPHA * vt + (1 - ALPHA) * vt_new
            vd = ALPHA * vd + (1 - ALPHA) * vd_new

        else:
            # v2 path: original smoothing and construction
        ...
        # Capture old positions before applying motion
        old_sq = np.array(square.pos, dtype=float)
        old_dot = np.array(dot.pos, dtype=float)

        # Apply motion with boundary clipping
        if target == "square":
            new_sq = confine(tuple(old_sq + vt))
            new_dot = confine(tuple(old_dot + vd))
        else:
            new_sq = confine(tuple(old_sq + vd))
            new_dot = confine(tuple(old_dot + vt))
        
        square.pos = new_sq
        dot.pos = new_dot

        # Compute REALIZED displacements (after clipping)
        vt_disp = np.array(new_sq, dtype=float) - old_sq if target == "square" else np.array(new_dot, dtype=float) - old_dot
        vd_disp = np.array(new_dot, dtype=float) - old_dot if target == "square" else np.array(new_sq, dtype=float) - old_sq

        # --- EVIDENCE (computed from realized displayed displacements) ---
        vm = np.array([dx, dy], dtype=float)
        mouse_speed = np.linalg.norm(vm) + 1e-9
        u_disp_t = vt_disp / (np.linalg.norm(vt_disp) + 1e-9)
        u_disp_d = vd_disp / (np.linalg.norm(vd_disp) + 1e-9)
        cos_T = float(np.dot(vm, u_disp_t)) / mouse_speed
        cos_D = float(np.dot(vm, u_disp_d)) / mouse_speed
...
# ========================= Version Note (v3_wen) =========================
# CHANGES vs v2:
# - Target/distractor motion now uses Wen-style ANGLE BLEND:
#     direction := rotate(mouse_dir toward template_dir by (1 - controlRatio) of shortest arc)
#     step length := mouse_speed (same for target and distractor)
# - Evidence now computed from DISPLAYED velocities (vt, vd), not raw templates.
# - Disabled distractor orthogonalization and heavy smoothing; light low-pass only (ALPHA=0.25).
# - QUEST 'prop' is used as controlRatio ∈ [0,1] without remapping.
# ========================================================================
